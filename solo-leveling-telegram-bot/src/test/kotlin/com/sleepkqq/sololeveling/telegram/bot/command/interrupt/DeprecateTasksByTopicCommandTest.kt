...
class DeprecateTasksByTopicCommandTest {

	private lateinit var userSessionService: UserSessionService
	private lateinit var command: DeprecateTasksByTopicCommand

	@BeforeEach
	fun setUp() {
		userSessionService = mock()
		command = DeprecateTasksByTopicCommand(userSessionService)
	}

	@Test
	fun `command should return deprecate_tasks_by_topic command string`() {
		// When/Then
		assertThat(command.command).isEqualTo("/deprecate_tasks_by_topic")
	}
	...
	@Test
	fun `createState should return DeprecateTasksByTopicState`() {
		// Given
		...
		assertThat(result).isInstanceOf(DeprecateTasksByTopicState::class.java)
	}
	...
	fun `handle should return Question when session is not in IdleState`() {
		...
		val message = mock<Message>()
		val session = mock<UserSession>()
		val nonIdleState = mock<DeprecateTasksByTopicState>()
