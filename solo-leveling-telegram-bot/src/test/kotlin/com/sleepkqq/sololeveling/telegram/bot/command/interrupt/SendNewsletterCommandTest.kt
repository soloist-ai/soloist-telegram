...
class SendNewsletterCommandTest {

	private lateinit var userSessionService: UserSessionService
	private lateinit var command: SendNewsletterCommand

	@BeforeEach
	fun setUp() {
		userSessionService = mock()
		command = SendNewsletterCommand(userSessionService)
	}

	@Test
	fun `command should return send_newsletter command string`() {
		// When/Then
		assertThat(command.command).isEqualTo("/send_newsletter")
	}
	...
	@Test
	fun `createState should return NewsletterNameState`() {
		// Given
		...
		assertThat(result).isInstanceOf(NewsletterNameState::class.java)
	}
	...
	fun `handle should return Question when session is not in IdleState`() {
		...
		val message = mock<Message>()
		val session = mock<UserSession>()
		val nonIdleState = mock<NewsletterNameState>()
