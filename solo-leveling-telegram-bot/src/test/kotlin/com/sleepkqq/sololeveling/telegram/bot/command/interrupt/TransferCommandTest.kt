...
class TransferCommandTest {
	...
	private lateinit var command: TransferCommand

	@BeforeEach
	fun setUp() {
		userSessionService = mock()
		command = TransferCommand(userSessionService)
	}

	@Test
	fun `command should return transfer command string`() {
		// When/Then
		assertThat(command.command).isEqualTo("/transfer")
	}
	...
	@Test
	fun `createState should return TransferAmountState`() {
		// Given
		...
		assertThat(result).isInstanceOf(TransferAmountState::class.java)
	}
	...
	fun `handle should return Question when session is not in IdleState`() {
		...
		val message = mock<Message>()
		val session = mock<UserSession>()
		val nonIdleState = mock<TransferAmountState>()
